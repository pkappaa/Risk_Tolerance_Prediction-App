...
model = joblib.load('questionnaire/ml_model/random_forest_model .pkl')
...
def calculate_risk_profile(form_data):
   ...
   # Load feature order from JSON
    with open('questionnaire/ml_model/features.json') as f:
        feature_order = json.load(f)        
       
    
    input_values = {}
    for col in feature_order:
        val = float(form_data[col])
        if col == 'INCOME':
            val = np.log1p(val) if val > 0 else 0
        elif col == 'NETWORTH':
            val = np.log1p(val) if val > 0 else 0
        
        input_values[col] = val

    # Δημιουργία DataFrame με σωστή σειρά
    ordered_values = [input_values[col] for col in feature_order]
    input_df = pd.DataFrame([ordered_values], columns=feature_order, dtype=np.float64)
    print(input_df)  
    
    
    risk_score = model.predict(input_df)[0]
    print("Risk Tolerance: ", risk_score)
...
def get_portfolio(risk_tolerance):
    ...
    # Define asset universe with more granular risk characteristics
    assets = [
